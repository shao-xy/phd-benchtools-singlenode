...
for thisBatch in thisBatchList:
    for tailBatch in tailBatchList:
        inodeList = []
        inodeMinuteList = []
        with open(DirPrefix+InodePrefix+thisBatch+tailBatch, 'r') as inodeFile:
        ...
        pos = 0
        with open(DirPrefix+InodePrefix+thisBatch+tailBatch+Suffix, 'w') as inodeMinuteFile:
